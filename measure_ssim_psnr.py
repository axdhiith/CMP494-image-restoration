...
def SSIMs_PSNRs(gtr_dir, gen_dir, im_res=(256, 256)):
    ...
    ssims, psnrs, mses, uiqms, uicms, uisms, uiconms = [], [], [], [], [], [], []
    for gtr_path, gen_path in zip(gtr_paths, gen_paths):
        ...
        if (gtr_f==gen_f):
            ...
            # get ui measures
            uiqm, uicm, uism, uiconm = getUIQM(np.array(g_im))
            uiqms.append(uiqm)
            uicms.append(uicm)
            uisms.append(uism)
            ...
            psnrs.append(psnr)
    return np.array(ssims), np.array(psnrs), np.array(uiqms), np.array(uicms), np.array(uisms), np.array(uiconms)
